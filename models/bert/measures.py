...
def f1_score(prediction, ground_truth):
    print('compare: '+prediction+'\t'+ground_truth)
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    groundtruth_answer = example.orig_answer_text
    groundtruths.append(groundtruth_answer)

  assert len(predictions)==len(groundtruths)

  f1 = computeF1(predictions, groundtruths)
  em = computeEM(predictions, groundtruths)

  tf.logging.info('Eval F1: '+str(f1) + ', EM: '+str(em))
